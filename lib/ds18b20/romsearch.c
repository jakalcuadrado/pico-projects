...
uint8_t ds18b20search( sensor_t * sensor, uint8_t *romcnt, uint8_t *roms, uint16_t buflen )
{
	uint8_t bit, temp, currom = 0;
	...
	do
	{
		...
		onewireWrite( sensor, DS18B20_COMMAND_SEARCH_ROM );
		...
		for ( i = 1; i; i <<= 1 )
		{
			...
			bit = 0;
			bit |= onewireReadBit( sensor );
			temp=onewireReadBit( sensor );
			bit |=  temp<< 1;
